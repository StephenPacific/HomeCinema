...
Extension version `0.3.8` adds the `v4` fast recovery envelope with quicker drift isolation, smoother fades, and a longer verified rejoin. It includes the bounded timeline-lock retries, manual lock recovery, room and per-Speaker volume controls, explicit **Use this computer** address action, and Controller output telemetry from the previous versions.
