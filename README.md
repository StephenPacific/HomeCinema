...
Current development baseline: room-sync engine `v9`, Chrome extension `0.3.13`. See the [V9 development log](docs/v9-development-log.zh-CN.md) for the implemented recovery algorithm, verification results, known limitations, and next test matrix.
...
Live WebRTC sessions use a four-phase startup: `Measuring`, `Locking`, `Armed`, and `Playing`. The extension and speakers establish their media paths while muted. The server waits for three consecutive RTP timing samples, selects a stable timing cohort, and chooses a fixed room target. With three or more Speakers, a single receiver more than 45 ms behind the median can be excluded when at least two thirds of the room form a stable cohort. Each faster Speaker fills its one-time gap with a deterministic Web Audio delay. Only after the combined browser and local delay is verified does the server schedule the shared start timestamp.

If a required Speaker misses timeline lock within the locking window, startup retries up to three times while affected outputs remain muted. Once playback has started, one unhealthy Speaker is isolated without stopping healthy outputs. Controller shows the failing receiver diagnostics and keeps manual retry controls available. Controller also provides a room volume slider for every output, including the capture computer, and an independent volume slider for each Speaker. Changes use a short gain ramp to avoid clicks and do not alter the locked playback timeline.

Speaker pages report a room-sync engine version. Version `v9` is required for the current timing cohort and automatic recovery behavior. Stale pages and superseded connections remain visible for diagnosis but are not counted as timing candidates and do not receive the live WebRTC track. Reloading the page upgrades the receiver without allowing an old tab to block the room at 90%.

Speakers continuously monitor WebRTC playout while keeping audible delay correction limited to 3 ms every two seconds, applied as a 1.8-second Web Audio ramp. A single 80 ms timeline jump, or three consecutive samples beyond 25 ms, isolates only that Speaker. While silent it can retune more frequently. Rejoining requires six consecutive samples within 8 ms, remains cancellable during a 1.2-second arming window, and then fades in over 1.2 seconds. A shared positive drift can trigger a coordinated room relock, while the stable cohort prevents one slow outlier from determining the new target.

If one Speaker remains in recovery for 8 seconds, engine v9 fades out and automatically rebuilds only that device's signaling and WebRTC path. Repeated recovery failures back off to 16 and 32 seconds, with a maximum of three attempts. A failed WebRTC transport uses a 2-second retry. Healthy Speakers continue playing. Browser audio contexts that require a local user gesture are never force-resumed by this mechanism.

Extension version `0.3.13` works with the `v9` room-sync engine and fixes both Windows address paths. A VMware current connection cannot replace the recommended WLAN Speaker QR link, and a previously saved same-machine VMware Controller service is automatically migrated to `127.0.0.1` after a verified local handshake. Remote coordination services keep their LAN address. The background capture status no longer overwrites an explicitly selected service address.
