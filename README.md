...
Extension version `0.3.7` adds bounded timeline-lock retries, manual lock recovery, room and per-Speaker volume controls, and an explicit **Use this computer** address action. The observation-only Controller output telemetry introduced in `0.3.6` remains available: while tab audio is active, the capture endpoint samples its AudioContext every 500 ms and reports a rolling 30-second window every two seconds. These measurements do not alter delay or playback; they distinguish stable offsets, latency jumps, and clock drift before a correction policy is chosen.
